...
package body Interfaces.C.Strings
is
   ...
   --  --  We can't use directly System.Memory because the
   ...
   function New_String (Str : String) return chars_ptr is
      ...
      --  It's important that this subprogram uses the heap directly to
      --  compute the result, and doesn't copy the string on the stack,
      --  otherwise its use is limited when used from tasks on large
      --  strings.
   ...
   --     --  ACATS cxb3010 checks that Constraint_Error gets raised when
   --     --  Length
   --     --  is 0. Seems better to check that Length is not null before
   --     --  declaring an array with size_t bounds of 0 .. Length - 1 anyway.
